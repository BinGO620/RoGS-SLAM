#!/bin/bash
# run_p9_static_ablate_3090.sh — P9 静态崩溃机制消融，3090 双卡（判据 p9_static_ablate_prereg.md）
# 用法: nohup bash scripts/run_p9_static_ablate_3090.sh > results/runs/P9/master_3090.log 2>&1 &
set -u
REPO=/home/jiangwenheng/cron/monogs-ours; cd "$REPO"
PY=/home/jiangwenheng/anaconda3/envs/monogs-ours-3090/bin/python
OUT=results/runs/P9/P9-ABLATE-3090
mkdir -p "$OUT"; DONE="$OUT/ablate.done"; : > "$DONE"
MAX_PARALLEL=2
# 头号嫌疑排最前，vanilla 锚点次之
RUNS="nodynkf:0 nodynkf:1 vanilla:0 vanilla:1 norobust:0 norobust:1 norelsig:0"
...
for entry in $RUNS; do
  ...
  sleep 20   # 让上一个先占住显存，避免选卡时两个挤同一张（exp26 踩过）
done
...
echo "P9_3090_ALL_DONE $(date) missing=$missing" >> "$DONE"
